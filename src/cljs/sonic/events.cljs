...
(defn calcOutcomeScore
  ...
  (let [{:keys [playerShip enemyShip]} db
        score (if (= pov :enemyShip)
                (- (calcShipStrength enemyShip) (calcShipStrength playerShip))
                (- (calcShipStrength playerShip) (calcShipStrength enemyShip)))]
...
(defn genActionVector
  ...
  (let [function (ACTION->FUNCTION (first action))]
    [action (changePhase (function cofx action) nil) []]))
...
(def ENEMY_ACTION_MAP
  {:missiles [:damageShip :targetSystem :playerShip :missiles MEAN_DICEROLL true]
   :lasers [:damageShip :targetSystem :playerShip :lasers MEAN_DICEROLL true]
   :repairBay [:repairShip :targetSystem :enemyShip]
   :shields [:chargeShields :enemyShip MEAN_DICEROLL]})
...
(defn updateActionList
 [shipType actionList]
 (apply concat (map updateAction2 (repeat shipType) actionList)))

(defn getAllActionList
  [db shipType]
  (->> (map systemInactive? ENEMY_ACTION_PREREQ_LIST (repeat (shipType db)))
       (remove true?)
       (map ENEMY_ACTION_MAP)
       (updateActionList shipType))) ;makes a list of possible actions for a ship

;makes array of all action vectors from an action list
(defn makeActionArray
  [cofx shipType]
  (->> (getAllActionList (:db cofx) shipType)
...
(defn updateNestedFuture
  [[action cofx future] steps]
  (if (= steps 0)
   (updateFuture [action cofx future])
   (let [[newAction newCofx newFuture] (updateFuture [action cofx future])]
     [newAction newCofx (mapv updateNestedFuture newFuture (repeat (dec steps)))])))
...
(defn chooseActionFromList
 [shipType actionList]
 (first (pickBestAction (map chooseAction (repeat shipType) actionList) shipType)))

(defn enemyChooseAction2
 [cofx]
 (let [choice (chooseActionFromList :enemyShip (last (updateNestedFuture [0 cofx 0] 1)))]
...
(rf/reg-event-fx
  ...
  (fn [cofx effects]
    ...
    {:db (:db cofx)
     :dispatch (enemyChooseAction2 cofx)}))
